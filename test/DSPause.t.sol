...
import {DSAuth, DSAuthority, DSPause} from "./DSPause.sol";
...
contract Constructor is DSPauseTest {
...
contract Schedule is DSPauseTest {
    ...
    function testFail_schedule_above_max_scheduled_txs() public {
        ...
        for (uint i = 0; i <= pause.maxScheduledTransactions(); i++) {
            bytes memory parameters = abi.encodeWithSignature("give(uint256)", i);
            pause.scheduleTransaction(usr, codeHash, parameters, eta);
        }
    }
    ...
    function testFail_schedule_eta_above_max_delay() public {
        ...
        bytes32      codeHash = extcodehash(usr);
        bytes memory parameters = abi.encodeWithSignature("get()");
        uint         eta = block.timestamp + pause.MAX_DELAY() + 1;
    ...
    function test_schedule_duplicate_transaction_after_execution() public {
        ...
        pause.scheduleTransaction(usr, codeHash, parameters, block.timestamp + pause.delay());
        bytes32 id = keccak256(abi.encode(usr, codeHash, parameters, block.timestamp + pause.delay()));
        assertTrue(pause.scheduledTransactions(id));
    }
...
contract Execute is DSPauseTest {
...
contract Abandon is DSPauseTest {
